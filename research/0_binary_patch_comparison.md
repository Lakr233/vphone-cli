...
- 2026-04-07 manual JB flow notes:
  - `make restore_get_shsh` and `make restore` succeeded reliably only after passing both `RESTORE_UDID` and `RESTORE_ECID` from `vm/udid-prediction.txt`.
  - without explicit restore identity, host-side `pymobiledevice3` device selection can be confused by another real USB-connected iPhone/iPad on the same Mac; docs should tell users to disconnect other devices or pass the explicit restore identity.
