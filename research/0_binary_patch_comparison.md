...
>
> **Frida support is opt-in.** JB-23b and JB-25b run only when firmware patching is invoked with `--frida` (`vphone-cli fw patch <vm> --variant jb --frida`, `patch-firmware ... --frida`, `vphone-cli vm create ... --frida`, or `make fw_patch_jb FRIDA=1`). For a fresh `vm create --frida`, the create orchestrator also passes `VPHONE_FRIDA=1` to the JB/EXP host install; it stages `https://build.frida.re/` as a Sileo/APT deb822 source, and first-boot setup installs package `re.frida.server`. Baseline JB/EXP outputs do not contain these relaxations or install Frida.
...
| 10  | Frida (`--frida`)          | Add `https://build.frida.re/` to Sileo/APT and install `re.frida.server` during first-boot JB finalization; retry until registered | - | - | opt-in |
