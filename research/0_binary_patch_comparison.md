...
>
> **Frida support is opt-in.** JB-23b and JB-25b run only when firmware patching is invoked with `--frida` (`vphone-cli fw patch <vm> --variant jb --frida`, `patch-firmware ... --frida`, `vphone-cli vm create ... --frida`, or `make fw_patch_jb FRIDA=1`). For a fresh `vm create --frida`, the create orchestrator also passes `VPHONE_FRIDA=1` to the JB/EXP host install. The host stages `https://build.frida.re/` as a Sileo/APT deb822 source and writes an explicit install marker. First-boot setup installs `re.frida.server` directly through APT, verifies the final dpkg state, and withholds its done marker until Frida is registered. Baseline JB/EXP outputs do not contain these relaxations or install Frida.
...
| JB-23b| B     | `patch_thread_set_state_core_registers` | inlined `thread_set_state_allowed()` core-register entitlement clause                              | Frida Stalker existing-thread support: NOP only the middle core-register entitlement failure edge; preserve exception-handler and fatal-PAC debug-state guards. Semantic string/call/three-gate matcher; enabled only by `--frida`. | `--frida` |
...
| JB-25b | B   | `patch_vm_map_delete_immutable_code`  | `vm_map_delete` permanent/immutable-code removal paths                                               | **Frida Stalker repeated-`VM_PROT_COPY` fix.** A successful CSM debugger association makes the entry `vme_permanent=1`; after write-then-flip it can be current `RW` but max `RWX`. XNU's debugger exception tests `entry->protection & VM_PROT_EXECUTE`, so the next fixed overwrite leaves the entry permanent and returns `KERN_PROTECTION_FAILURE`. Retarget both compiled execute gates from packed current-X bit 9 to max-X bit 13, preserving branch sense/target (`tbz` and `tbnz` shapes). Semantic matcher requires `ldr wFlags,[entry,#0x38]` → permanent bit 19 → immutable-code remove bit 6 → developer-mode bit-0 gate; exactly two hits or fail closed. No offsets/VAs/register numbers are hardcoded; replacements come from the decoded test-bit branch operands via `ARM64Encoder`. GDB-verified on 26.4 UUID `784F88D3-2AF5-3D4A-AB00-365607646E44`: exact entry `0x088ab9c0` = RW/max-RWX/permanent/CSM/user-debug; current-X gate falls through to `mov w0,#2`. This is deliberately narrower than swallowing every outer `kr=2`; enabled only by `--frida`. | `--frida` |
...
| 10  | Frida (`--frida`)          | Stage `https://build.frida.re/`; first boot installs `re.frida.server` through APT, verifies dpkg state, and retries on later boots until registered | - | - | opt-in |
