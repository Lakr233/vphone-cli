...
# The guest binaries this variant installs — TweakLoader.dylib and vpregister —
# are cross-compiled at build time (scripts/guest_binaries.mk) and shipped.
GUEST_BIN=""
...
check_prerequisites() {
    ...
    # the two guest binaries below are cross-compiled at build time rather than
...
# TweakLoader — arm64 + arm64e, built by scripts/guest_binaries.mk. This used to
# compile TweakLoader.m through `xcrun --sdk iphoneos`, which meant a user who
# only wanted to run a VM still had to install Xcode. Signing stays, because it
# uses this VM's own signcert.
stage_tweakloader() {
    local src="$GUEST_BIN/TweakLoader.dylib"
    local out="$TEMP_DIR/TweakLoader.dylib"
    [[ -f "$src" ]] || die "Missing prebuilt TweakLoader.dylib at $src — run 'make build'"
    cp -f "$src" "$out"
    guest_sign "$out"
    echo "$out"
}

# vpregister — registers JB app bundles via the containerized LaunchServices
...
stage_vpregister() {
    local src="$GUEST_BIN/vpregister"
    local out="$TEMP_DIR/vpregister"
    [[ -f "$src" ]] || die "Missing prebuilt vpregister at $src — run 'make build'"
    cp -f "$src" "$out"
    guest_sign_ent "$out" "$SCRIPT_DIR/vphoned/entitlements.plist"
    echo "$out"
}
...
# libzstd and decompresses in-process, so the check below is the only one left.
