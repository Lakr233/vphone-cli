...
final class GuestHTTPHandler: ChannelInboundHandler, RemovableChannelHandler, @unchecked Sendable {
    ...
    private func handle(_ head: HTTPRequestHead, body: Data, channel: Channel) {
        ...
        if head.method == .GET && path == "/openapi.json" {
            Self.send(APIReply(status: 200, data: OpenAPISpec.data), on: channel)
            return
        }
        ...
        do {
            ...
            let queue = request.method == "icli.execute" ? IcliCommand.queue : GuestAPI.queue
            queue.async { job.run() }
        } catch {
    ...
    private static func route(_ verb: HTTPMethod, path: String) throws -> String {
        ...
        case (.POST, "/v1/icli/execute"): "icli.execute"
