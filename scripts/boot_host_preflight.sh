...
echo "allowlist, which is what 'make amfi_allow' installs, that is the EXPECTED result,"
echo "not a contradiction: it allows only the binaries it was given, and the debug"
echo "control above is a fresh copy in a temp directory with its own CDHash."
echo "'make amfi_allow' covers this build; it needs root and is not run for you."
