...
echo "something is permitting this particular binary and not others — and with an"
echo "allowlist-style AMFI bypass, such as amfidont, that is the EXPECTED result, not"
echo "a contradiction: it allows only the paths and CDHashes it was given, and the"
echo "debug control above is a fresh copy in a temp directory with its own CDHash."
echo "This project ships no bypass and starts none; running one is yours to do."
echo "'make amfi_command' prints the command line, with this build's CDHash filled in."
