...
struct VPhoneSignStyleTests {
    ...
    /// The flags `--use-ldid` hands over have to be the same request, or the
    /// fallback quietly signs something else.
    @Test("the ldid arguments match the options")
    func ldidArguments() {
        #expect(VPhoneLdid.arguments(
            for: .init(identifier: nil), entitlementsPath: nil, identityPath: nil
        ) == ["-S"])
        #expect(VPhoneLdid.arguments(
            for: .init(identifier: "com.apple.seputil", mergesExisting: true),
            entitlementsPath: nil, identityPath: "/tmp/signcert.p12"
        ) == ["-S", "-M", "-K/tmp/signcert.p12", "-Icom.apple.seputil"])
        #expect(VPhoneLdid.arguments(
            for: .init(entitlements: Data([1]), mergesExisting: true),
            entitlementsPath: "/tmp/e.plist", identityPath: nil
        ) == ["-S/tmp/e.plist", "-M"])
    }

    /// Handing ldid an identity it cannot see would sign ad-hoc and say
    /// nothing, which is the one way a fallback is worse than no fallback.
    @Test("--use-ldid refuses an identity it was not given the .p12 for")
    func escapeHatchRefusesAnIdentityWithoutItsFile() throws {
        try #require(VPhoneSignLdidHarness.ldid != nil, "ldid is not installed")
        let identity = try VPhoneSignIdentity(
            pkcs12: Data(contentsOf: VPhoneSignIdentityTests.p12), password: ""
        )
        let source = try #require(VPhoneSignLdidHarness.corpus.first)
        let directory = try VPhoneSignLdidHarness.temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        ...
        #expect(throws: VPhoneSignError.self) {
            try VPhoneSigner.sign(fileAt: file, options: .init(
                identifier: "binary", identity: identity, usesExternalLdid: true
            ))
        }
    }

    /// Going through the external tool must land on the same file, or the
    /// fallback is not a fallback.
    @Test("--use-ldid produces what signing here produces")
    func escapeHatchAgrees() throws {
        try #require(VPhoneSignLdidHarness.ldid != nil, "ldid is not installed")
        for source in VPhoneSignLdidHarness.corpus.prefix(4) {
            ...
            let viaLdid = try VPhoneSignLdidHarness.copy(source, into: directory, as: name)
            try VPhoneSigner.sign(fileAt: viaLdid, options: .init(
                identifier: name, entitlements: VPhoneSignParityTests.sampleEntitlements,
                usesExternalLdid: true
            ))
            let here = try VPhoneSignLdidHarness.copy(source, into: directory, as: "here-\(name)")
            try VPhoneSigner.sign(fileAt: here, options: .init(
                identifier: name, entitlements: VPhoneSignParityTests.sampleEntitlements,
                usesExternalLdid: false
            ))

            let left = try Data(contentsOf: viaLdid), right = try Data(contentsOf: here)
