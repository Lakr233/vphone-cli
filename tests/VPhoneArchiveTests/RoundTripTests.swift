...
struct RoundTripTests {
    ...
    @Test(
        "tar round trip preserves contents, nesting and symlinks",
        arguments: [
    ...
    func roundTrip(compression: VPhoneArchiveCompression) throws {
    ...
    @Test("the compressor actually ran")
    func compressionIsApplied() throws {
        let source = try Self.makeTree()
        let plain = Self.scratch("plain").appendingPathExtension("tar")
        let compressed = Self.scratch("zstd").appendingPathExtension("tzst")
        defer {
            for url in [source, plain, compressed] { try? FileManager.default.removeItem(at: url) }
        }
    ...
    @Test("member paths are relative, even when the source is reached through a symlink")
    func memberPathsAreRelative() throws {
        let source = try Self.makeTree()
    ...
    @Test("decompress unwraps a single stream without unpacking it")
    ...
    @Test("a member that escapes the destination is refused")
    func pathEscapeIsRefused() throws {
    ...
    private func rewriteFirstMemberName(of archive: URL, to name: String) throws {
        ...
        for i in 0..<100 { bytes[i] = 0 }
        for (i, byte) in Array(name.utf8).enumerated() { bytes[i] = byte }

        for i in 148..<156 { bytes[i] = UInt8(ascii: " ") }   // checksum field reads as spaces
        let sum = (0..<512).reduce(0) { $0 + Int(bytes[$1]) }
        let octal = String(format: "%06o", sum)
        for (i, byte) in Array(octal.utf8).enumerated() { bytes[148 + i] = byte }
        bytes[154] = 0
