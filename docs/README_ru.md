...
**Вариант B — оставить SIP включённым (ослабленным только для отладки) и позволить `vphone-cli` открывать окно на каждый запуск** (в остальное время AMFI остаётся включённым).
...
Затем перезагрузитесь в macOS. Больше настраивать нечего: у `vphone-cli` нет entitlements, поэтому он запускается всегда, и при старте гостя он замечает, что amfid не принимает `vphone-vm`, открывает окно с помощью `vphone-letmein` (один запрос sudo) и закрывает его, как только гость запущен.

Сделать это вручную — имеет смысл при работе над `vphone-vm`, где один sudo лучше, чем запрос на каждый запуск:

```
make letmein          # открыть     (sudo)
make letmein_status   # проверить
make letmein_off      # закрыть
```

> **Честно о том, что это делает.** Это глобальный переключатель, а не allowlist: пока окно открыто, amfid сообщает, что *каждая* проверяемая им подпись действительна и подписана Apple. Это нельзя сузить до одного пути или одного бинарника — решение на уровне отдельной проверки требует приватных отладочных entitlements Apple. Сузить можно время, поэтому автоматический путь держит окно открытым только на время одного запуска. Кроме того, это существует лишь в памяти: перезагрузка сбрасывает состояние.
>
> `vphone-letmein` заменяет старый помощник `amfidont`, который был pip-пакетом и больше не используется.
...
**`zsh: killed ./vphone-vm`** — ограничения AMFI/debug не обойдены; см. Предварительные требования (`amfi_get_out_of_my_way=1` или пусть `vphone-cli` сам откроет окно). Обратите внимание: с самим `vphone-cli` такого произойти не может — у него нет entitlements, поэтому если убивают *его*, дело в чём-то другом.
