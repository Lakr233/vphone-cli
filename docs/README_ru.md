...
**Вариант B — оставить SIP включённым (ослабленным только для отладки) и самостоятельно запускать `amfidont` на время запуска** (в остальное время и для всех бинарников, которые вы не разрешили, AMFI остаётся включённым).
...
Затем перезагрузитесь в macOS.

[`amfidont`](https://github.com/zqxwce/amfidont) — это отдельный инструмент, который вы устанавливаете и запускаете сами. **Этот проект его не поставляет, не устанавливает, не запускает и им не управляет**, и от него не зависит: `vphone-cli` лишь замечает, что `vphone-vm` был убит, и сообщает, что нужно запустить. Никакой зависимости от Python в этом репозитории от этого не появляется.

Устанавливайте через Python от Apple. Python из Homebrew откажет по PEP 668, а сам инструмент делает re-exec `python3` из Xcode, поэтому Xcode должен быть установлен:

```
xcrun python3 -m pip install --user amfidont
# попадает в ~/Library/Python/3.9/bin — добавьте в $PATH
```

Получите cdhash от `vphone-vm`. Именно он несёт entitlements; у `vphone-cli` их нет и он запускается всегда, так что его cdhash не нужен:

```
VPHONE_BIN="$PWD/.build/vphone-cli.app/Contents/MacOS"   # там, где лежит vphone-vm
codesign -dv --verbose=4 "$VPHONE_BIN/vphone-vm" 2>&1 | sed -n 's/^CDHash=//p' | head -1
```

Запустите демон в отдельном терминале и оставьте его работать:

```
sudo amfidont daemon --path "$VPHONE_BIN" --cdhash <cdhash> --spoof-apple --verbose
```

Затем запускайте как обычно из другого терминала — `vphone-cli vm launch myphone`.

`--path` / `-p` и `--cdhash` / `-c` можно указывать несколько раз, и оба объединяются со списками из `~/.amfidont/paths` и `~/.amfidont/cdhashes`. Запишите записи туда один раз через `amfidont add-path <dir>` и `amfidont add-cdhash <hash>` (`remove-path` / `remove-cdhash` отменяют), и дальше вся команда сводится к `sudo amfidont daemon --spoof-apple`. Учтите: пересборка меняет cdhash, поэтому список только из cdhash устаревает после каждого `make build`, а список по `--path` — нет.

> **Честно о том, что это разрешает.** Это allowlist по префиксу пути и по cdhash: для всего, что вы не указали, amfid продолжает проверять подписи. `--spoof-apple` заставляет разрешённые бинарники выглядеть подписанными Apple — именно это и нужно приватным entitlements PV=3. Исключение — `--allow-all`: с ним *каждая* проверяемая amfid подпись считается действительной, пока демон работает. В любом случае всё это существует лишь в памяти: остановите демон или перезагрузитесь, и amfid вернётся к обычному поведению.
>
> `amfidont` пришёл на смену `vphone-letmein`, который открывал своё окно записью в `__TEXT` amfid. На хосте, где `sysctl vm.cs_system_enforcement` равен 1 — измерено на macOS 27.0 (26A428), arm64e, ровно с теми настройками `csrutil`, что выше, — ядро убивает amfid из-за этой «грязной» страницы и забирает с собой гостя, а сам sysctl доступен только для чтения. `amfidont` управляет amfid через LLDB, поэтому его точки останова живут в отладочных регистрах процессора и страница вообще не пишется; именно поэтому он работает при включённом enforcement там, где патч не работает.
...
**`zsh: killed ./vphone-vm`** — ограничения AMFI/debug не обойдены; см. «Ослабление SIP/AMFI» (`amfi_get_out_of_my_way=1` — вариант A, либо запущенный `amfidont` с `vphone-vm` в allowlist — вариант B). Обратите внимание: с самим `vphone-cli` такого произойти не может — у него нет entitlements, поэтому если убивают *его*, дело в чём-то другом.
