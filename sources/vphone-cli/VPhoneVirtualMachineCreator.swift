...
public struct VPhoneVirtualMachineCreator {
    ...
    public func run(_ options: Options) throws {
        ...
        // never sees it.
        var sudoEnvironmentExtras: [String: String] = [:]
        ...
        if let password = options.sudoPassword, !password.isEmpty {
            ...
            sudoEnvironmentExtras = ["SUDO_ASKPASS": script.path, "SUDO_PASSWORD": password]
            if preloadSudoCredential(env: sudoEnvironmentExtras, verbosity: v) {
            ...
            if sudo?.succeeded != true {
                throw VPhoneVirtualMachineCreationError.sudoPasswordRequired
            }
        }
    ...
    private func runCustomFirmwareInstall(
        ...
        let code = try VPhoneCustomFirmwareInstaller.elevate(
            ...
            rootPopup: usePopup, environment: sudoEnvironmentExtras,
        )
