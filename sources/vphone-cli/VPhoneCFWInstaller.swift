...
struct VPhoneCFWInstaller {
    let bundle: URL
    let resources: VPhoneResources
    let forceDSCMaxSlide: Bool

    private var executable: URL { VPhoneResources.runningExecutable() }
    private var fm: FileManager { .default }

    static func elevate(
        bundle: URL,
        resources: VPhoneResources,
        forceDSCMaxSlide: Bool,
        rootPopup: Bool,
        environment: [String: String] = [:],
        verbose: Bool = true
    ) throws -> Int32 {
        let exe = VPhoneResources.runningExecutable()
        var args = ["cfw", "install-root", bundle.path, "--resources", resources.base.path]
        if forceDSCMaxSlide { args.append("--force-dsc-maxslide") }
        if geteuid() == 0 {
            try VPhoneCFWInstaller(bundle: bundle, resources: resources,
                forceDSCMaxSlide: forceDSCMaxSlide).run()
            return 0
        }
        if rootPopup {
            var env = environment
        ...
        args.insert(contentsOf: ["-E", exe.path], at: 0)
        if env["SUDO_ASKPASS"] != nil { args.insert("-A", at: 0) }
    ...
    func run() throws {
        ...
        guard let baseDisk = attached.split(whereSeparator: \.isNewline).first?
            .split(whereSeparator: \.isWhitespace).first.map(String.init),
              baseDisk.hasPrefix("/dev/disk") else {
        ...
        guard let plist = try PropertyListSerialization.propertyList(
            from: Data(info.utf8), format: nil) as? [String: Any],
              let container = plist["APFSContainerReference"] as? String,
              container.hasPrefix("disk") else {
        ...
        if let uid = ProcessInfo.processInfo.environment["SUDO_UID"].flatMap(UInt32.init) {
            let signed = bundle.appendingPathComponent(".vphoned.signed")
            if fm.fileExists(atPath: signed.path) { _ = chown(signed.path, uid, gid_t.max) }
        }
    ...
    private func installMounted(system: URL, data: URL, work: URL) throws {
        guard let restore = try fm.contentsOfDirectory(at: bundle, includingPropertiesForKeys: [.isDirectoryKey])
            .first(where: { $0.lastPathComponent.contains("Restore") &&
                fm.fileExists(atPath: $0.appendingPathComponent("iPhone-BuildManifest.plist").path) }) else {
        ...
        if version.hasPrefix("27.") { try patchCampo(system: system, work: work) }
    }
    ...
    private func installCryptexes(restore: URL, system: URL, work: URL) throws {
        ...
        if !((try? fm.contentsOfDirectory(atPath: os.path))?.isEmpty == false &&
            ...
            let paths = try CFWDaemons.cryptexPaths(
    ...
    private func installVphoned(system: URL, work: URL) throws {
        ...
        let signed = bundle.appendingPathComponent(".vphoned.signed")
        if fm.fileExists(atPath: signed.path) { try fm.removeItem(at: signed) }
    ...
    private func patchMachO(system: URL, work: URL, path: String, verb: String,
        ...
        if !fm.fileExists(atPath: backup.path) { try fm.copyItem(at: target, to: backup) }
        ...
        guard let source = try VPhoneSigner.entitlements(ofFileAt: target)
            .first(where: { !$0.isEmpty }),
              var plist = try PropertyListSerialization.propertyList(
                from: source, format: nil) as? [String: Any] else {
    ...
    private func patchCampo(system: URL, work: URL) throws {
        ...
        try fm.copyItem(at: target, to: staged)
        try VPhoneSigner.sign(fileAt: staged,
            options: .init(entitlements: try Data(contentsOf: ent, options: .mappedIfSafe), mergesExisting: true))
        try replace(staged, at: target, mode: 0o755)
    }
    ...
    private func productVersion(system: URL) throws -> String {
        ...
        guard let value = try PropertyListSerialization.propertyList(
            from: Data(contentsOf: plist, options: .mappedIfSafe), format: nil) as? [String: Any],
              let version = value["ProductVersion"] as? String else {
...
struct VPhoneCFWInstallRootCommand: ParsableCommand {
    ...
    @Flag(name: .customLong("force-dsc-maxslide")) var forceDSCMaxSlide = false

    func run() throws {
        try VPhoneCFWInstaller(
