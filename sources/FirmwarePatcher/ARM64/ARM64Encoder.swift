...
public enum ARM64Encoder {
    ...
    /// Encode TBZ/TBNZ (test bit and branch) while preserving semantic operands.
    ///
    /// `bit` selects a bit in the source register; values below 32 encode the W-register
    /// form used by packed VM map-entry flags. The target must be 4-byte aligned and
    /// within the signed 14-bit branch range (+/-32 KB).
