// DSCChunkSet.swift — Flat virtual-address access over a chunked dyld shared cache.
//
...
// independent reference for this layer — see `DSCFoundationTests`.
//
...
public struct DSCWriteSpan: Sendable, Hashable {
    ...
    public static func byte(at vma: UInt64) -> DSCWriteSpan {
        DSCWriteSpan(vma: vma, length: 1)
    }
...
public final class DSCChunkSet {
    ...
    private var writeLog: [DSCWriteSpan] = []
    private var loadedLocalSymbols: DSCLocalSymbolTable?
    ...
    public init(directory: URL, architecture: String = "arm64e") throws {
        ...
        guard !chunkURLs.isEmpty else {
            throw DSCError.noChunksFound(directory: directory.path)
        }
        ...
        guard !collected.isEmpty else {
            throw DSCError.noMappingsParsed(directory: directory.path)
        }
    ...
    public func fileRange(of span: DSCWriteSpan) throws -> (chunkURL: URL, range: Range<Int>) {
        guard let run = contiguousRun(from: span.vma) else {
            throw DSCError.addressNotMapped(span.vma)
        }
        guard UInt64(span.length) <= run.available else {
            throw DSCError.addressSpanCrossesChunk(vma: span.vma, length: span.length)
        }
    ...
    public func bytesAtVMA(_ vma: UInt64, length: Int) throws -> Data {
        ...
        let (chunkURL, range) = try fileRange(of: DSCWriteSpan(vma: vma, length: length))
        let data = try Self.read(url: chunkURL, offset: UInt64(range.lowerBound), length: length)
        guard data.count == length else {
            throw DSCError.shortRead(vma: vma, wanted: length, got: data.count)
        }
        return data
    }
    ...
    public func readAtVMA(_ vma: UInt64, length: Int, allowShort: Bool = false) throws -> Data {
        ...
        guard let run = contiguousRun(from: vma) else {
            throw DSCError.addressNotMapped(vma)
        }
        let wanted: Int
        if run.available >= UInt64(length) {
            ...
            throw DSCError.addressSpanCrossesChunk(vma: vma, length: length)
        }
    ...
    ///   `DSCCodeSignature.reattest(in:modifiedSpans:)` consumes. The same span
    ...
    @discardableResult
    public func write(at vma: UInt64, _ data: Data) throws -> DSCWriteSpan {
        guard !data.isEmpty else { return DSCWriteSpan.byte(at: vma) }
        let span = DSCWriteSpan(vma: vma, length: data.count)
    ...
    /// first.
    public var recordedWrites: [DSCWriteSpan] {
    ...
    public func localSymbolTable() throws -> DSCLocalSymbolTable {
        try stateLock.withLock {
            ...
            let table = try DSCLocalSymbolTable(url: localSymbolsURL)
    ...
    /// which image a symbol came from should use `DSCSymbolResolver` instead.
    ///
