// DSCCodeSignature.swift — Page-hash re-signing for dyld shared cache chunks.
//
// Why this exists
// ---------------
...
// `patch_amfi_cdhash_in_trustcache` — `KernelJBPatchAmfiTrustcache` in this
...
// derived from the spans that were actually written, and `DSCChunkSet` records
// those itself.
...
public struct DSCChunkCodeDirectory: Sendable {
...
public struct DSCSlotReattestation: Sendable {
...
public struct DSCPageReference: Sendable, Hashable {
    public let chunkURL: URL
    public let pageIndex: Int
}
...
public struct DSCReattestationSkip: Sendable {
...
public enum DSCCodeSignature {
    ...
    public static func readCodeDirectory(ofChunk url: URL) throws -> DSCChunkCodeDirectory? {
        let head = try DSCChunkSet.read(url: url, offset: 0, length: 0x100)
        ...
        let superHead = try DSCChunkSet.read(url: url, offset: signatureOffset, length: 12)
        ...
        let directoryHead = try DSCChunkSet.read(url: url, offset: blobOffset, length: 44)
    ...
    public static func reattest(
        ...
        var skipped: [DSCReattestationSkip] = []

        func skip(_ reason: DSCReattestationSkip.Reason) {
            skipped.append(DSCReattestationSkip(reason: reason))
        ...
        for span in modifiedSpans {
            ...
            } catch DSCError.addressNotMapped {
                skip(.addressNotMapped(vma: span.vma))
                continue
            } catch DSCError.addressSpanCrossesChunk {
                skip(.spanNotAddressable(vma: span.vma, length: span.length))
                continue
            }

            let directory: DSCChunkCodeDirectory?
        ...
        var records: [DSCSlotReattestation] = []
        var alreadyAttested: [DSCPageReference] = []
    ...
    ///
    /// The path a patcher should take: write through `DSCChunkSet.write(at:_:)`
...
// File-private: `CFWMachOCodeSignature` declares the same helper for the
