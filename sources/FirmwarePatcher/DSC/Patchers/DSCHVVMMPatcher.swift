// DSCHVVMMPatcher.swift — the `kern.hv_vmm_present` user-mode cstring mangle.
//
...
// `KernelEXPPatcher.patchHvVmmRename`, in
// `Kernel/EXPPatches/KernelEXPPatchHvVmmRename.swift` — renames the sysctl OID
...
// here is therefore followed by `DSCCodeSignature`, which re-hashes the pages
// that were actually dirtied. This patcher never names those pages itself —
// `DSCChunkSet` records each write and the re-attestation pass consumes the
...
public enum DSCHVVMMPatcher {
    ...
    /// What it becomes — the same letter `KernelEXPPatchHvVmmRename` renames the
    ...
    public static func patch(
        ...
        for vma in pristineSites {
            ...
            if dryRun {
                plannedSpans.append(DSCWriteSpan.byte(at: writeVMA))
            } else {
        ...
        for vma in alreadyMangledSites {
            ...
            let span = DSCWriteSpan(vma: vma, length: needle.count)
            if dontPatchSet.contains(installName) {
                log?(
        ...
        // JB kernel patch `KernelJBPatchAmfiTrustcache` short-circuits AMFI's
        // per-image trust-cache lookup.
        //
        // The write spans come from `DSCChunkSet` itself rather than from a list
    ...
    /// same place.
    static func classify(_ vma: UInt64, in chunks: DSCChunkSet) -> String? {
