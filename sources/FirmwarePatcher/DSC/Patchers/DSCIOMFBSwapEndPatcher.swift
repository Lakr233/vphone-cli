// DSCIOMFBSwapEndPatcher.swift — Make IOMobileFramebuffer's SwapEnd payload
...
// independent reference — `DSCIOMFBSwapEndTests` runs the Python on one clone of
...
// shelling out to `ipsw dyld symaddr`. Here that is `DSCSymbolResolver`, which
...
public struct DSCIOMFBSwapEndPatch: Sendable {
...
public enum DSCIOMFBSwapEndPatcher {
    ...
    /// patchers over one `DSCChunkSet` does not re-hash the previous one's
    ...
    public static func patch(
        ...
        if dryRun {
            ...
            reattestation = try DSCCodeSignature.reattestRecordedWrites(in: chunks, log: log)
