// DSCMaxSlidePatcher.swift — Clamp the dyld shared cache's recorded maxSlide so
...
//
// So `DSCCodeSignature.reattestRecordedWrites(in:)` is *not* called here, and
// after this patcher runs the main chunk's page 0 hashes to something its code
// slot no longer claims. `DSCMaxSlidePatcherTests` pins that state rather than
// leaving it as a thing someone has to remember.
//
// To keep a later batch re-attestation from silently undoing that decision,
// this patcher opens its own `DSCChunkSet` over the directory instead of
...
public enum DSCMaxSlidePatcher {
    // MARK: - Constants
    ...
    public enum Outcome: Sendable, Equatable {
        ...
        var reason: String? {
            ...
            case let .overflow(combined, region):
                "overflow: span+maxSlide \(DSCMaxSlidePatcher.hex(combined)) > "
                    + "region \(DSCMaxSlidePatcher.hex(region))"
            case let .forced(combined, region):
                "forced: span+maxSlide \(DSCMaxSlidePatcher.hex(combined)) fits "
                    + "region \(DSCMaxSlidePatcher.hex(region)) but --force set"
            }
        }
    }
    ...
    public static func patch(
        ...
        let chunks = try DSCChunkSet(directory: chunksDirectory, architecture: architecture)
        ...
        // No `DSCCodeSignature.reattestRecordedWrites(in: chunks)` here. See the
    ...
    static func headerVMA(of chunks: DSCChunkSet) throws -> UInt64 {
