// DSCIOMFBForceKernPatcher.swift — Force IOMobileFramebuffer's present onto the
...
//
// Companion kernel patch: `KernelJBPatcher.patchIomfbSwapEndVariableSize` /
...
// are resolved by NAME through `DSCSymbolResolver` (export trie plus the cache's
...
// dirtied is re-attested through the span log `DSCChunkSet` keeps.
//
// Port of `scripts/patchers/cfw_patch_iomfb_force_kern.py`, which stays the
// independent reference — `DSCIOMFBForceKernTests` runs the Python on one clone
...
public enum DSCIOMFBForceKernPatcher {
    ...
    public static func patch(
        ...
        if records.isEmpty {
            ...
            let result = try DSCCodeSignature.reattestRecordedWrites(in: chunks, log: log)
    ...
    private static func force(_ site: Site, chunks: DSCChunkSet) throws -> PatchRecord {
        ...
        let span = DSCWriteSpan(vma: entry.publicAddress, length: 4)
